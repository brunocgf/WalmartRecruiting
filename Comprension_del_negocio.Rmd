...
A continuación se muestra la preparación para los datos de prueba

```{r}

m_upc <- mode(test$Upc)
m_dep <- mode(test$DepartmentDescription)
m_line <- mode(test$FinelineNumber)

test <- test %>% 
  mutate(Upc = replace_na(Upc,m_upc),
         DepartmentDescription = replace(DepartmentDescription,DepartmentDescription=='NULL',m_dep),
         FinelineNumber = replace_na(FinelineNumber,m_line))
```
